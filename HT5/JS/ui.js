...
function togglePlayer() {
  ...
  function takeMove(event, player) {
    var unit = field.getUnit(event.target.id);
    if (unit.player) {
      return
    }
    if (unit.setPoint(player)) {
      if (unit.position[1] == field.firstRow) {
        field.extend('top');
      } else if (unit.position[1] > field.firstRow + field.rows  -2) {
        field.extend('bottom');
      } else if (unit.position[0] == field.firstCol) {
        field.extend('left');
      } else if (unit.position[0] > field.firstCol + field.cols - 2) {
        field.extend('right');
      } 
      field.trySurround(unit);

      togglePlayer();
    }
  }

//убрать после добавления Settings
  var game = {};
  game.settings = {};
  game.settings.score = 'area';
//убрать после добавления Settings
  ...
  // to Move
  Field.prototype.trySurround = function(startUnit) {
    var pathTree = PathTree.build(startUnit);
  ///////////////////////////
 ...
 /////////////////////////////
  ...
  function PathTree(startUnit) {
    this.levels = [];
    this.levels[0] = [new PathTreeNode(startUnit)];
    this.pathes = [];
  ...
  Path.prototype.render = function() {
    ...
    for (var i = 0; i < this.innerUnits.length; i++) {
      ...
      if (this.innerUnits[i].player) {
        this.innerUnits[i].surrounded = this.innerUnits[i].player == currentPlayer ? false : true;
      }
    }
  }
  Path.prototype.analyze = function() {
    ...
    for (var i = 0; i < this.innerUnits.length; i++) {
      ...
      if (player == currentPlayer && !this.innerUnits[i].surrounded) {
        alliedDots++;
      } else if (player) {
        enemyDots++;
      }
    ...
    this.scores = game.settings.score == 'area' ? area : enemyDots;
  }
  ...
  function PathTreeNode(unit, parentNode, angle, changeAngle) {
    this.unit = unit;
    this.angle = angle;
  ...
  PathTreeNode.prototype.createChildNode = function(ally) {
    var node = new PathTreeNode(ally.unit, this);
    if (!node.allias) {
      return false;
    }
  ...
  PathTreeNode.prototype.getSVGPath = function() {
  ...
  PathTreeNode.prototype.checkNoBorder = function() {
  ...
  PathTreeNode.prototype.isSectorsExistsInUnit = function(changeAngle) {
  ...
  PathTreeNode.prototype.render = function() {
  ...
  Unit.prototype.getAllias = function() {
    ...
    for (var angle in DIRECTIONS) {
      var unit = field.getUnit(DIRECTIONS.getPosition(this.position, angle));
  ...
  Unit.prototype.paint = function(svgPath) {
    ...
    svgPathElem.classList.add(currentPlayer);
